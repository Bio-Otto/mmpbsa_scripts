...
class SystemLoader:
    ...
    def prepare_ligand(self, lig):
        with self.logger("prepare_ligand") as logger:
            ...
            # Save ligand to PDB with proper formatting
            pdb_content = Chem.MolToPDBBlock(oemol)
            header = "REMARK  Generated by MMGBSA package\n"
            header += "REMARK  Ligand structure\n"
            header += "TITLE     LIGAND\n"
            
            with open(f'{self.dirpath}/lig.pdb', 'w') as f:
                f.write(header)
                f.write(pdb_content)
    ...
    def prepare_protein(self, protein):
        with self.logger("prepare_protein") as logger:
            ...
            # Save protein to PDB with proper formatting
            pdb_content = Chem.MolToPDBBlock(protein)
            header = "REMARK  Generated by MMGBSA package\n"
            header += "REMARK  Protein structure\n"
            header += "TITLE     PROTEIN\n"
            
            with open(f'{self.dirpath}/apo.pdb', 'w') as f:
                f.write(header)
                f.write(pdb_content)
    ...
    def _create_complex_pdb(self, protein, ligand):
        ...
        with self.logger("_create_complex_pdb") as logger:
            try:
                ...
                # Save the complex to PDB with better formatting
                pdb_content = Chem.MolToPDBBlock(combined)
                
                # Add proper PDB header
                header = "REMARK  Generated by MMGBSA package\n"
                header += "REMARK  Complex structure\n"
                header += "TITLE     PROTEIN-LIGAND COMPLEX\n"
                
                # Write the file with header
                with open(f'{self.dirpath}/com.pdb', 'w') as f:
                    f.write(header)
                    f.write(pdb_content)
        ...
        with self.logger("__setup_system_im") as logger:
            try:
                ...
                with working_directory(self.dirpath):
                    ...
                    # Create topology from the complex PDB (now using relative path)
                    pdb_file = app.PDBFile('com.pdb')
